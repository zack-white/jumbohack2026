/**
 * Minimal PCAP parser for browser: extracts IPv4 addresses, packet stats,
 * and hostnames from DHCP (option 12).
...
export function parsePcap(buffer: ArrayBuffer): HostInfo[] {
  const view = new DataView(buffer)
  if (buffer.byteLength < PCAP_GLOBAL_HEADER_SIZE) {
    throw new Error('File too small to be a valid PCAP')
  }

  const magic = readU32(view, 0, false) // magic is always read big-endian to detect format
  const littleEndian = magic === 0xd4c3b2a1 || magic === 0xd5c3b2a1
  const isNano = magic === 0xa1b2c3d5 || magic === 0xd5c3b2a1
  const validMagic =
    magic === 0xa1b2c3d4 || magic === 0xa1b2c3d5 || magic === 0xd4c3b2a1 || magic === 0xd5c3b2a1
  if (!validMagic) {
  ...
  while (offset + PCAP_PACKET_HEADER_SIZE <= buffer.byteLength) {
    ...
    const tsMs =
      tsSec * 1000 + (isNano ? tsUsec / 1e6 : Math.floor(tsUsec / 1000))
    ...
    // DHCP: extract hostname from client messages (UDP dst port 67)
  ...
  // Attach hostnames to hosts (from DHCP)
